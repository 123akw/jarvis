...
SearXNG Base URL 只接受两类地址：通过安全传输校验的公开 `https://` URL，或精确的本机 `http://127.0.0.1:18888` / `http://[::1]:18888`（允许末尾 `/` 规范化）。其他 HTTP、其他回环端口和任何私网地址均拒绝；本机特例不能复用于自定义模型中转。生产部署审计发现 BT-Panel 已占用 8888，且候选 18888 的 IPv4、IPv6、防火墙与 Docker 引用均为 0，因此本机 SearXNG 特例随部署统一迁移到 18888。
